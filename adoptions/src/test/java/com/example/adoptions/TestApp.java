...
class TestContainers {
    ...
    static class AdoptionServiceContainer extends GenericContainer<AdoptionServiceContainer> {
        public AdoptionServiceContainer() {
            super(DockerImageName.parse("adoption-service"));
            addExposedPort(8081);
    ...
    @Bean
    AdoptionServiceContainer adoptionService() {
        return new AdoptionServiceContainer();
    }

    @Bean
    public DynamicPropertyRegistrar adoptionServiceProperties(AdoptionServiceContainer container) {
        return (properties) ->
...
class Initializer {
    @Autowired
    // @RestartScope
    void dogsInDb(VectorStore vectorStore, DogRepository dogRepository) {
        ...
        if (System.getProperty("INIT") == null) {
            ...
            var doguments = dogRepository.findAll().stream().map(
                    dog -> new Document("id: %s, name: %s, description: %s".formatted(dog.id(), dog.name(), dog.description()))
            ).toList();
